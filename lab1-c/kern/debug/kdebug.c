 ...
 *
 * The x86 stack pointer, namely esp,栈顶 points to the lowest location on the stack
 * that is currently in use. Everything below that location in stack is free.高地址向低地址 Pushing
 * a value onto the stack will invole decreasing the stack pointer and then writing
 * the value to the place that stack pointer pointes to. And popping a value do the
 * opposite.
 *
 * The ebp (base pointer) 栈底 register, in contrast, is associated with the stack
...
print_stackframe(void) {
    ...
    while(i<STACKFRAME_DEPTH && ebp!=0){//宏定义20
      ...
      * (3) from 0 .. STACKFRAME_DEPTH 这是个全局变量吗？
