...
.code16                                             # Assemble for 16-bit mode
    cli                #屏蔽中断                             # Disable interrupts
    7c00:	fa                   	cli    
    cld                  #传输方向                           # String operations increment
    7c01:	fc                   	cld    
...
seta20.1:                                      #等待8042键盘控制器不忙
    ...
    testb $0x2, %al                                 #从0x64端口中读入一个字节到al中  
    7c0c:	a8 02                	test   $0x2,%al
    jnz seta20.1                                    #al的第2位为0，则跳出循环 确保缓冲区位空
    7c0e:	75 fa                	jne    7c0a <seta20.1>

    movb $0xd1, %al                                #将0xd1写入al中   # 0xd1 -> port 0x64
    7c10:	b0 d1                	mov    $0xd1,%al
    outb %al, $0x64                                #将0xd1写入到0x64端口中 # 0xd1 means: write data to 8042's P2 port
    7c12:	e6 64                	out    %al,$0x64

00007c14 <seta20.2>:

seta20.2:                                            #等待8042键盘控制器不忙
    inb $0x64, %al                                   #从0x64端口中读入一个字节到al中    # Wait for not busy(8042 input buffer empty).
    ...
    movb $0xdf, %al                               #将0xdf入al中       # 0xdf -> port 0x60
    7c1a:	b0 df                	mov    $0xdf,%al
    outb %al, $0x60                               #将0xdf入到0x64端口中，打开A20       # 0xdf = 11011111, means set P2's A20 bit(the 1 bit) to 1
...
readseg(uintptr_t va, uint32_t count, uint32_t offset) {
    ...
    uint32_t secno = (offset / SECTSIZE) + 1;
    7c8a:	8d 41 01             	lea    0x1(%ecx),%eax
readseg(uintptr_t va, uint32_t count, uint32_t offset) {
    7c8d:	83 ec 08             	sub    $0x8,%esp
    uintptr_t end_va = va + count;
    7c90:	89 7d ec             	mov    %edi,-0x14(%ebp)
...
inb(uint16_t port) {
    ...
    7c93:	bb f7 01 00 00       	mov    $0x1f7,%ebx
    uint32_t secno = (offset / SECTSIZE) + 1;
...
outb(uint16_t port, uint8_t data) {
    ...
    7d02:	81 c6 00 02 00 00    	add    $0x200,%esi
