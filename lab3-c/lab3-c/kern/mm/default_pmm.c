 ...
 * `list_next`返回next, `list_prev`返回之后. 增加删除，之后之前。
 ...
 * valid有效的. P.S. In function `pmm_init` (in pmm.c), the bit `PG_reserved` of
 ...
 */
free_area_t free_area; //使用结构完成对空闲块的管理。
...
default_init_memmap(struct Page *base, size_t n) {
    assert(n > 0);//断言判断是否大于0；
    struct Page *p = base;
    for (; p != base + n; p ++) {//初始化n块物理页
        assert(PageReserved(p));//检查此页是否为保留页
        p->flags = p->property = 0;// 把中间的页的标志位清零，已经分配，
...
default_alloc_pages(size_t n) {
    assert(n > 0);
    if (n > nr_free) {//请求分配的页个数大于空闲页总数，返回
        return NULL;
    }
    ...
    if (page != NULL) {
        ...
        if (page->property > n) {//如果分配的页的大小大于所需大小，分割；
            struct Page *p = page + n;//改变p的基址，p成为空闲页
	    //do
            p->property = page->property - n;
            SetPageProperty(p);////把flag设为1 头页
            list_add_after(&(page->page_link), &(p->page_link));//加入空闲链表
        }
        list_del(&(page->page_link));// 分配页删除
        nr_free -= n;//
        ClearPageProperty(page);//清空分配页
    }
    return page;
}
//页释放函数
